...
public class AnalysisScreen extends AppCompatActivity {
    ...
    static final String photoType = "perp";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int min_rgb_in[] = {0,0,0};
        int min_hsv_in[] = {0,0,0};
        int min_rgb_out[] = {255,255,255};
        int min_hsv_out[] = {255,255,255};

        int max_rgb_in[] = {0,0,0};
        int max_hsv_in[] = {0,0,0};
        int max_rgb_out[] = {255,255,255};
        int max_hsv_out[] = {255,255,255};

        int min_rgb_start[] = {0,0,0};
        int min_hsv_start[] = {0,0,0};
        int min_rgb_end[] = {0,0,0};
        int min_hsv_end[] = {0,0,0};
        ...
        for(int i = 1; i <= 55; i++){
            ...
            for(int j = 0; j < 3; j++){
                ...
                if(min_rgb_start[j] > data.HistDataRgb[j][0]) min_rgb_start[j] = (int)data.HistDataRgb[j][0];
                if(max_rgb_start[j] < data.HistDataRgb[j][0]) max_rgb_start[j] = (int)data.HistDataRgb[j][0];

                if(min_hsv_start[j] > data.HistDataHsv[j][0]) min_hsv_start[j] = (int)data.HistDataHsv[j][0];
                if(max_hsv_start[j] < data.HistDataHsv[j][0]) max_hsv_start[j] = (int)data.HistDataHsv[j][0];

                if(min_rgb_end[j] > data.HistDataRgb[j][255]) min_rgb_end[j] = (int)data.HistDataRgb[j][255];
                if(max_rgb_end[j] < data.HistDataRgb[j][255]) max_rgb_end[j] = (int)data.HistDataRgb[j][255];

                if(min_hsv_end[j] > data.HistDataHsv[j][255]) min_hsv_end[j] = (int)data.HistDataHsv[j][255];
                if(max_hsv_end[j] < data.HistDataHsv[j][255]) max_hsv_end[j] = (int)data.HistDataHsv[j][255];
            }
        }
        StringBuilder sb = new StringBuilder();

        sb.append("RGB\nVals:\nMin\n");
        for(int i = 0; i < 10; i++) sb.append(i + ": " + min_r_val_rgb[i] + " " + min_g_val_rgb[i] + " " + min_b_val_rgb[i] + "\n");
        sb.append("Max\n");
        for(int i = 0; i < 10; i++) sb.append(i + ": " + max_r_val_rgb[i] + " " + max_g_val_rgb[i] + " " + max_b_val_rgb[i] + "\n");

        sb.append("\nHSV\nVals:\nMin\n");
        for(int i = 0; i < 10; i++) sb.append(i + ": " + min_r_val_hsv[i] + " " + min_g_val_hsv[i] + " " + min_b_val_hsv[i] + "\n");
        sb.append("Max\n");
        for(int i = 0; i < 10; i++) sb.append(i + ": " + max_r_val_hsv[i] + " " + max_g_val_hsv[i] + " " + max_b_val_hsv[i] + "\n");
